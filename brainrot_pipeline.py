...
async def process_video(url, output_dir="output", subway_video_path=None, progress_queue=None):
    ...
    1. Download using downloader.py
    2. Extract all highlights using auto-editor via highlights.py
    3. Format for mobile using video_formatter.py
    4. Stack with background video
    5. Add subtitles using movie.py
    """
    ...
    # Create temp directory for intermediate files that will be cleaned up
    ...
    highlight_extractor.min_clip_duration = 10  # Minimum clip duration in seconds
    highlight_extractor.max_clip_duration = 40  # Maximum clip duration in seconds
    # Remove max clips limit to extract all possible highlights
    highlight_extractor.silent_threshold = 0.04  # Adjusted threshold for better detection
    ...
    # Report total highlights to the UI
    if progress_queue:
    ...
    if not subway_video_path:
        # Try to find Subway Surfer video
        possible_paths = [
    ...
    # Process all highlight clips with improved parallelism
    final_outputs = []

    # Create a process pool for CPU-bound operations (like transcription)
    process_pool = ProcessPoolExecutor(max_workers=min(os.cpu_count(), 4))

    # Create a semaphore with dynamic concurrency based on system resources
    concurrency_limit = min(os.cpu_count() * 2, 16)  # More aggressive but controlled scaling
    semaphore = asyncio.Semaphore(concurrency_limit)

    # Pipeline stages with separate semaphores for different resource types
    io_semaphore = asyncio.Semaphore(concurrency_limit * 2)  # IO can handle more concurrency
    cpu_semaphore = asyncio.Semaphore(os.cpu_count())  # CPU-bound operations limited by cores

    # Helper function with improved error handling and timeouts
    async def run_subprocess_async(cmd, check=True, timeout=300):  # 5 minute timeout default
        try:
    ...
    async def process_highlight_clip(highlight_clip, clip_index):
        ...
        async with semaphore:
            ...
            # Run ffmpeg command asynchronously to format for mobile
            try:
                ...
                # Make height even for video encoding
                if target_height % 2 != 0:
                    target_height += 1
                
                print(f"  Scaling to: {target_width}x{target_height} (maintaining aspect ratio, height made even)")
            ...
            # STEP 4: Generate transcription for subtitles
            ...
            audio_path = create_audio(mobile_clip)
            ...
            if not audio_path:
                ...
                try:
                    ...
                    # This is already CPU-bound so we need to run it with proper concurrency control
                    wordlevel_info = transcribe_audio(whisper_model, audio_path)
            ...
            if subway_video_path and os.path.exists(subway_video_path):
                ...
                # Loop the background video to match the clip duration
                try:
                    loop_cmd = [
                        "ffmpeg", "-y",
                        "-stream_loop", "-1",  # Loop indefinitely
                        "-i", subway_video_path,
                        "-t", str(clip_duration),  # Cut to the clip duration
                        "-c:v", "libx264", "-crf", "23",
            ...
            # Add subtitles to the mobile clip
            subtitled_clip, _ = add_subtitle(
                mobile_clip,                # Video file
                audio_path,                 # Audio file
                "9x16",                     # Vertical format
                "top",                      # Subtitle position
                "#FFFF00",                  # Highlight color (yellow)
                6.0,                        # Font size
                0.3,                        # Background opacity
                12,                         # Max chars per line
                "#FFFFFF",                  # Text color (white)
                wordlevel_info,             # Word-level transcription data
                output_dir_subtitles        # Output directory
            )
            ...
            # STEP 7: Stack videos (highlight on top, background on bottom)
            ...
            try:
                if use_background and background_clip:
                    # Get actual dimensions of the subtitled clip
                    probe_cmd = [
                        "ffprobe", 
                        "-v", "error", 
                        "-select_streams", "v:0", 
                    ...
                    # Calculate the height for the Subway Surfers video
                    total_height = 1920
                    
                    # Calculate optimal proportion - make Subway Surfers larger
                    # Allocate 65-75% of space to Subway Surfers if possible
                    top_max_percent = 0.35  # Top video maximum percentage
                    top_min_percent = 0.25  # Top video minimum percentage
                    ...
                    # Ensure bottom height is even
                    if bottom_height % 2 != 0:
                        bottom_height -= 1
                        top_ideal_height += 1  # Adjust top height
                        
                    print(f"Optimized heights: Main={top_ideal_height}px ({top_ideal_height/total_height:.1%}), Subway={bottom_height}px ({bottom_height/total_height:.1%})")
                    
                    # Scale top video (may need to crop it slightly to fit optimal height)
                    top_video_scaled = temp_dir / f"top_scaled_{clip_basename}.mp4"
                    
                    # Scale and potentially crop top video to fit ideal height
                    top_scale_cmd = [
                        "ffmpeg", "-y",
                    ...
                    # Scale bottom video (subway surfers) to focus on the BOTTOM part instead of top
                    ...
                    # Calculate crop offset to focus on bottom part of the video
                    # This extracts the BOTTOM portion by setting the Y offset appropriately
                    scale_factor = max(1080 / bg_width, bottom_height / bg_height)
                    scaled_height = int(bg_height * scale_factor)
                    y_offset = max(0, scaled_height - bottom_height)  # This ensures we get the bottom portion

                    # Calculate the new height for the bottom video
                    new_height = bottom_height + scaled_height

                    # Scale and crop the bottom video
                    bottom_scale_cmd = [
                        "ffmpeg", "-y",
                        "-i", str(background_clip),
                        "-vf", f"scale=1080:{new_height}:force_original_aspect_ratio=increase,crop=1080:{bottom_height}:0:{y_offset}",
                        "-c:v", "libx264", "-crf", "23",
                        "-an",  # No audio for bottom video
                        str(bottom_video_scaled)
                    ]
                    await run_subprocess_async(bottom_scale_cmd)
                    
                    # Create a 4px gradient transition between videos instead of black space
                    ...
                    # Now stack the videos vertically with the gradient in between
                    stack_cmd = [
                        "ffmpeg", "-y",
                        ...
                        "-map", "[v]",  # Use the stacked video
                        "-map", "0:a",  # Use audio from the first input (top video)
                        "-c:v", "libx264", "-crf", "23",
                        "-c:a", "aac", "-b:a", "192k",
                        str(final_path)
                    ]
                    ...
                    # Check if stacked video was created successfully
                    ...
                    # Calculate padding to reach 9:16 aspect ratio
                    target_height = 1920
                    pad_height = target_height - top_height
                    
                    # Ensure total height is even
                    if (top_height + pad_height) % 2 != 0:
                        pad_height += 1
                        
                    print(f"Adding {pad_height}px of padding to reach 9:16 aspect ratio")
                    
                    # Use ffmpeg to add black padding to the bottom to reach 9:16 aspect ratio
                    pad_cmd = [
                        "ffmpeg", "-y",
                        "-i", subtitled_clip,
            ...
            try:
                # Only keep the optimized file in the main output directory
                optimize_cmd = [
                    "ffmpeg", "-y", 
                    "-i", str(final_path),
                    "-movflags", "+faststart",
                ...
                if optimized_path.exists() and optimized_path.stat().st_size > 0:
                    ...
                    # Notify about completion of this clip
                    if progress_queue:
                        await progress_queue.put({
                            "type": "completed",
                            "highlight": clip_index+1
                ...
                # Notify about completion of this clip despite error
                if progress_queue:
                    await progress_queue.put({
                        "type": "completed",
                        "highlight": clip_index+1
    ...
    # Create and run all highlight processing tasks in parallel
    ...
    for i, clip in enumerate(highlight_clips):
        task = asyncio.create_task(process_highlight_clip(clip, i))
        tasks.append(task)
    ...
    try:
        # Keep only the final output files and delete the temp directory with all intermediate files
        if temp_dir.exists():
